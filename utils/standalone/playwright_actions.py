...
class PlaywrightUtils:
    ...
    def new_browser(self, headless=True, persistent=True, user_data_dir=None, cdp_port=None, viewport=None, channel=None):
        ...
        #
        # channel: e.g. 'msedge' or 'chrome' to launch the real installed browser instead
        # of Playwright's bundled Chromium. Matters for sites relying on native Windows
        # SSO (WAM/Windows Hello for Business) - confirmed the bundled Chromium gets no
        # SSO at all (redirects to a real login form) even with a signed-in Windows
        # session, while the real Edge channel silently authenticates, even with a
        ...
        if persistent:
            ...
            browser = self._playwright.chromium.launch(headless=headless, args=launch_args, channel=channel)
    ...
    lib_demo_params = [
        {'key': 'a', 'name': 'New Browser', 'function': 'new_browser', 'inputs': [
            ...
            {'label': 'Browser channel (e.g. msedge)', 'name': 'channel', 'type': str, 'default': None, 'allow_empty': True},
        ]},
