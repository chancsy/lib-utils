...
############################################################################
# LAN Monitor (UDP Broadcast)
#
# Design notes:
#   - UDP broadcast stays within the subnet (Layer 2). Routers do NOT forward
#     broadcasts, so traffic is invisible outside the local network segment.
#     Low IT footprint: ~100 bytes per ping cycle on a non-standard port (47777).
#   - DHCP clients: broadcast eliminates the need to know client IPs.
#     Clients self-identify by hostname in every reply.
#   - "Truly offline" confirmation: a rebooted machine sends a heartbeat
#     immediately on startup. Sustained silence (no PONG + no HB beyond
#     offline_timeout) means the machine is genuinely offline — not just an
#     IP change from DHCP renewal.
#   - offline_timeout must be set greater than heartbeat_interval to avoid
#     false-positive offline alerts.
#
# Quick start:
#   Server (dashboard PC):
#       server = LanMonitorServer(watch=['PC-01', 'PC-02'])
#       server.start()
#       server.print_status()
#
#   Client (monitored PC, run at start of test script):
#       client = LanMonitorClient(status_callback=lambda: 'running')
#       client.start()
############################################################################

class LanMonitorClient:
    """
    Runs on each monitored PC. Listens for broadcast pings from the server
    and replies with its hostname + status. Also sends periodic heartbeats
    so the server stays aware even between pings.
    """
    DEFAULT_PORT = 47777
    _MSG_PING = 'LMON_PING'
    _MSG_PONG = 'LMON_PONG'
    ...
    def __init__(self, port=None, heartbeat_interval=0, status_callback=None):
        """
        port               : UDP port (must match server). Default 47777.
        heartbeat_interval : Seconds between unsolicited heartbeat broadcasts.
                             Default 0 (disabled). Set to e.g. 30 to enable.
                             Heartbeats help the server recover faster after a
                             restart, but increase subnet traffic — leave at 0
                             in enterprise networks where broadcast footprint
                             should be minimised. The ping/PONG mechanism alone
                             is sufficient for normal monitoring.
        status_callback    : Optional callable() returning a status string,
                             e.g. lambda: 'running' or lambda: my_script_state.
        """
    ...
    def _listen_loop(self):
        """Receive broadcast pings and reply directly to the sender (server)."""
            ...
            while not self._stop.is_set():
                try:
                    ...
                    if msg.startswith(self._MSG_PING):
                        ...
                        sock.sendto(reply, ('<broadcast>', self.port))  # broadcast so server's listen socket (port 47777) receives it
    ...
    def _heartbeat_loop(self):
        """Broadcast a heartbeat at regular intervals. No-op when heartbeat_interval is 0."""
...
class LanMonitorServer:
    """
    Dashboard server. Sends UDP broadcast pings, collects PONGs and heartbeats
    from clients, and tracks online/offline status by hostname.
    Works regardless of client DHCP IP changes.
    """
    DEFAULT_PORT = 47777
    _MSG_PING = 'LMON_PING'
    _MSG_PONG = 'LMON_PONG'
    ...
    def __init__(self, port=None, ping_interval=10, offline_timeout=60, watch=None):
        """
        port            : UDP port (must match client). Default 47777.
        ping_interval   : Seconds between broadcast pings. Default 10.
        offline_timeout : Seconds of silence before marking a host offline.
                          Should be greater than client heartbeat_interval. Default 60.
        watch           : Optional list of hostnames to track (case-insensitive).
                          When set, replies from any other host are silently ignored.
                          Watched hosts that have not yet replied appear in get_status()
                          as offline stubs so you always know the full expected fleet.
                          Example: watch=['PC-LAB01', 'PC-LAB02']
        """
        self.port = port or self.DEFAULT_PORT
        self.ping_interval = ping_interval
        self.offline_timeout = offline_timeout
        self.watch = {h.upper() for h in watch} if watch else None  # normalised set, O(1) lookup
        self.hostname = socket.gethostname()
        self._stop = threading.Event()
        self._lock = threading.Lock()
        self._clients = {}  # {hostname: {'status', 'last_seen', 'ip'}}
        self._threads = []

    def _listen_loop(self):
        """Receive PONGs and heartbeats from clients."""
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
        sock.settimeout(1.0)
            ...
            while not self._stop.is_set():
                try:
                    ...
                    if len(parts) >= 2 and parts[0] in (self._MSG_PONG, self._MSG_HB):
                        ...
                        if hostname == self.hostname:  # ignore own broadcasts
                            continue
                        if self.watch and hostname.upper() not in self.watch:  # filter to watched list
                            continue
                        with self._lock:
    ...
    def _ping_loop(self):
        """Broadcast a ping at regular intervals."""
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
        try:
            while not self._stop.is_set():
    ...
    def get_status(self):
        """
        Returns dict keyed by hostname:
            {hostname: {'status': str, 'last_seen': float|None, 'ip': str|None,
                        'is_online': bool, 'seconds_since_seen': float|None}}
        When watch is set, hosts in the watch list that have not yet replied are
        included as offline stubs (last_seen=None, ip=None).
        """
        now = time.time()
        with self._lock:
            result = {}
            # seed watched-but-silent hosts so the full expected fleet is always visible
            if self.watch:
                for h in self.watch:
    ...
    def print_status(self):
        """Print a human-readable dashboard table to stdout."""
